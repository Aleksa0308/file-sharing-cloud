...
public class AppConfig {
	...
	public static int WEAK_TOLERANCE;
	public static int STRONG_TOLERANCE;
	...
	public static void readConfig(String configName, int serventId){
		...
		try {
			WEAK_TOLERANCE = Integer.parseInt(properties.getProperty("weak_tolerance"));
		} catch (NumberFormatException e) {
			timestampedErrorPrint("Problem reading weak_tolerance. Exiting...");
			System.exit(0);
		}

		try {
			STRONG_TOLERANCE = Integer.parseInt(properties.getProperty("strong_tolerance"));
		} catch (NumberFormatException e) {
			timestampedErrorPrint("Problem reading strong_tolerance. Exiting...");
			System.exit(0);
		}
		...
		try {
			CLOUD_PATH = properties.getProperty("cloud_path");
			if (CLOUD_PATH == null || CLOUD_PATH.isEmpty()) {
				timestampedErrorPrint("cloud_path not specified or empty. Exiting...");
				System.exit(0);
			}
		} catch (Exception e) {
			timestampedErrorPrint("Problem reading cloud_path. Exiting...");
			System.exit(0);
		}
